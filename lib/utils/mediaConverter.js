...
export class MediaConverter {
  static async convertAudioToWhatsAppFormat(audioData, originalMimeType = 'audio/webm') {
    ...
    const tempOutputPath = join(tmpdir(), `output_${Date.now()}.ogg`);
    ...
    try {
      ...
      console.log(`Converting audio: ${inputExt} -> ogg, size: ${audioBuffer.length} bytes, mime: ${originalMimeType}`);
      
      // Convert to WhatsApp-compatible OGG/Opus
      // WebM to OGG/Opus conversion
      const command = `ffmpeg -i "${tempInputPath}" -c:a libopus -b:a 48k -ar 48000 -ac 1 -f ogg "${tempOutputPath}" -y`;
  ...
  static async ensureWhatsAppImageFormat(imageData, mimetype) {
    ...
    try {
      ...
      await execAsync(`ffmpeg -i "${tempInputPath}" -qscale:v 2 -f image2 "${tempOutputPath}" -y`);
